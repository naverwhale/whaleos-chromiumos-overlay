...
inherit cros-unibuild cmake toolchain-funcs
...
src_configure() {
	...
	# DPTF has a few variables which are set but not used. Allow Clang to
	# complain about this, but sidestep the build errors.
	append-flags -Wno-error=unused-but-set-variable
	local mycmakeargs=( -DCHROMIUM_BUILD=YES )
	use x86 && mycmakeargs+=( -DBUILD_ARCH=32bit )
	use amd64 && mycmakeargs+=( -DBUILD_ARCH=64bit )
	sed -i 's|-D_FORTIFY_SOURCE=2|-D_FORTIFY_SOURCE=3|g' \
		"${S}/DPTF/Linux/CMakeLists.txt" \
		"${S}/ESIF/Products/ESIF_CMP/Linux/Makefile" \
		"${S}/ESIF/Products/ESIF_UF/Linux/Makefile" \
		"${S}/ESIF/Products/ESIF_WS/Linux/Makefile" \
		"${S}/IPF/Linux/Makefile" \
		|| die
	cmake_src_configure
}
