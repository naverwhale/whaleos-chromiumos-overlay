...
IUSE="dlc gphoto2 ipv6 snmp systemd threads usb v4l xinetd zeroconf"
...
# 2.9 MiB / 4 KiB = 725 Blocks: Where 4KiB is the block size that DLC considers
# 725 * 1.3 ~= 950 Blocks -> DLC recommends 130% of the DLC size
# Setting blocks to 1024 to account for future backend growth or if additional
# backends are added
...
src_prepare() {
	...
	eapply "${FILESDIR}"/sane-backends-extras-1.2.1-remove-redundancies.patch
...
multilib_src_configure() {
	...
	econf \
		--disable-locking \
		--sysconfdir="$(dlc_add_path "etc")" \
		--libdir="$(dlc_add_path "usr/lib64")" \
		"$(use_with gphoto2)" \
