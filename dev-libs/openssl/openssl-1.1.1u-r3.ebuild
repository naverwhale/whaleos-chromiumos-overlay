...
LICENSE="openssl"
SLOT="PITA/1.1" # .so version of libssl/libcrypto
if [[ ${PV} != *_pre* ]] ; then
	KEYWORDS="*"
fi
...
multilib_src_install() {
	newlib.so libcrypto.so libcrypto.so.1.1
	newlib.so libssl.so libssl.so.1.1

	into "/usr/$(get_libdir)/engines-1.1"
	dolib.so engines/afalg.so
	dolib.so engines/capi.so
	dolib.so engines/padlock.so
}
