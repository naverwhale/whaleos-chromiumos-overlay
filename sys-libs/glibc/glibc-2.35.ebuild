...
setup_target_flags() {
	...
	# CrOS' compiler wrapper automatically infers `--sysroot=` and a few
	# other args, like library paths. We want to skip this, as it may lead
	# to us using libraries we don't want to use with glibc (e.g.,
	# llvm-libunwind): b/290144273
	#
	# Only add this flag for cross-compilation, since the host wrapper
	# doesn't do any of this.
	is_crosscompile && append-flags "--cros-skip-wrapper-sysroot-autodetection"
...
glibc_do_src_install() {
	...
	if [[ ${SYMLINK_LIB} == "yes" ]] && [[ ! -e ${ED}/$(alt_prefix)/usr/lib ]] ; then
		dosym $(get_abi_LIBDIR ${DEFAULT_ABI}) $(alt_prefix)/usr/lib
	fi
