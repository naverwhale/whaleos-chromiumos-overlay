...
DEPEND="${RDEPEND}
	app-arch/unzip
"
# bazel-5.3.0-r2 removed the symlink from /usr/bin/bazel to /usr/bin/bazel-5.
RDEPEND+="
	!<=dev-util/bazel-5.3.0-r1
"
...
src_prepare() {
	einfo "Applying upb fix"
	(cd "${S}" && "${FILESDIR}/apply_upb_fix.sh") || die
	default
