...
cros_pre_src_prepare_patches() {
	# Use mkstemp64 for large file support.
	# https://bugs.gentoo.org/902901
	sed -i -e "s/ret = mkstemp(template);/ret = mkstemp64(template);/" "${S}/openbsd-compat/mktemp.c" || die
}
...
cros_pre_src_configure_openssh_flags() {
	...
	"
	# Argument hack since we use /mnt/empty instead of /var/empty.
	rmdir() {
		if [[ "$*" == "${ED}"/var/empty ]]; then
			set -- "${ED}"/mnt/empty
		fi
		$(type -P rmdir) "$@"
	}
