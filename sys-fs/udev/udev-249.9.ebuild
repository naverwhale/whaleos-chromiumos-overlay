...
src_prepare() {
	local PATCHES=(
		...
		"${FILESDIR}"/CHROMIUM-50-udev-default.rules-Disable-REMOVE_CMD-su.patch
		"${FILESDIR}"/CHROMIUM-Don-t-read-udev-rules-from-run.patch
		"${FILESDIR}"/CHROMIUM-Use-the-video-group-for-drm-kfd-udmabuf.patch
		"${FILESDIR}"/CHROMIUM-udev-Drop-VMware-USB-mouse-workaround.patch
		"${FILESDIR}"/CHROMIUM-udev-bypass-v4l_id-query-for-mipi-camera-re.patch

		"${FILESDIR}"/BACKPORT-udev-check-existence-of-property-before-use.patch
		"${FILESDIR}"/BACKPORT-udev-builtin-path_id-split-out-add_id_tag.patch
		"${FILESDIR}"/BACKPORT-udev-add-USB-revision-in-ID_PATH.patch
	)
...
multilib_src_compile() {
	# ChromeOS-specific
	touch ${S}/tools/chromiumos/__init__.py || die
...
multilib_src_install() {
	...
	if multilib_is_native_abi ; then
		...
		newsbin udevadm udevd # TODO: Remove this copy, b/300084033
...
multilib_src_install_all() {
	...
	doins "${FILESDIR}/61-keyboard-chromeos.hwdb"

	# Remove some large files that are not needed in ChromeOS
	rm "${ED}/$(get_udevdir)/hwdb.d"/{20-OUI.hwdb,20-pci-vendor-model.hwdb} || die
