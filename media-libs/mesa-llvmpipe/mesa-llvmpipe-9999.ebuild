...
VIDEO_CARDS="${INTEL_CARDS} ${RADEON_CARDS} freedreno llvmpipe mach64 mga nouveau r128 radeonsi savage sis softpipe tdfx via virgl vmware iris"
...
src_prepare() {
	...
	eapply "${FILESDIR}"/0001-WhaleOS-build-with-python3.8.patch
...
src_install() {
	...
	local gallium_drivers_files=( nouveau_dri.so r300_dri.so r600_dri.so msm_dri.so swrast_dri.so iris_dri.so )
