...
src_prepare() {
	# Apply patch with changes made after the LTS branch was created.
	# In chromium/src the path starts with 'components/policy' but in the local
	# checkout it is 'cloud/policy', hence the '-p' and '--directory' arguments
	# (to strip 'components/' and add 'cloud/', respectively).
	eapply -p2 --directory=cloud/ "${FILESDIR}/0001-devicehardwarevideodecodingenabled-policy.patch"
	eapply_user
}
