...
BDEPEND="
	...
	net-misc/curl
"
case "${PREBUILT_VERSION}" in
	"1.69.0"|"1.70.0")
		# b/300477249: the prior rust-bootstrap was linked against an older version
		# of openssl, so we need that to be present during the build.
		BDEPEND+=" dev-libs/openssl:PITA"
	;;
esac
...
src_prepare() {
	...
	for version in "${RUSTC_VERSION_SEQUENCE[@]}"; do
		...
		if [[ "${version}" > "1.70" ]]; then
			eapply -p1 "${FILESDIR}/${PN}-uname-maxsplit.patch"
		fi
