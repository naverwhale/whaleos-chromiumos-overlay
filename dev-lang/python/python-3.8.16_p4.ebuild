...
src_compile() {
	...
	if use pgo_generate; then
		# Opting into `--pgo-extended` generates a fuller profile at the
		# potential cost of taking much longer. In reality, it doesn't
		# do much today, but we might as well opt in in case it's more
	...
	# in _sysconfigdata*
	emake CPPFLAGS= CFLAGS= LDFLAGS= "${extra_make_args[@]}"
