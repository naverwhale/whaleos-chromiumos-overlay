...
inherit cros-llvm cros-constants git-r3 python-any-r1 toolchain-funcs cros-workon
...
if [[ -n "${INCLUDE_PROFDATA_IN_SRC_URI}" ]]; then
	...
	SRC_URI+="
		rust_profile_frontend_use? ( ${PROFDATA_PREFIX}/rust-pgo-${PV}${PROFDATA_SUFFIX}-frontend.profdata.xz )
		rust_profile_llvm_use? ( ${PROFDATA_PREFIX}/distfiles/rust-pgo-${PV}${PROFDATA_SUFFIX}-llvm.profdata.xz )
	"
fi
